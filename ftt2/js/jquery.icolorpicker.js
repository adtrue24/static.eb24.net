...
var iColorPicker_created = 0;
(function($) {
	var imageUrl = 'editor_images/color.png';	
	function iColorPicker(elem, conf) {
		var self = this;
		var imageUrl = conf.imageUrl;
        if (iColorPicker_created == 0) {
            ...
            iColorPicker_created = 1;
        }
        jQuery('#colorPreview').css({
            'height': '50px'
        });
        if(jQuery(elem).val()) jQuery(elem).css('color', '#'+jQuery(elem).val()); 
        jQuery(elem).after('<a href="javascript:void(null)" id="icp_' + elem.id + '"><img src="' + imageUrl + '" style="border:0;margin:0 0 0 3px" align="absmiddle" ></a>');
        jQuery('#icp_'+elem.id).click(function() {
        	var id = elem.id;
        	var eICP = jQuery(this).position();
		    jQuery("#iColorPicker").css({
		        'top': (eICP.top - jQuery("#iColorPicker").height()-jQuery('#'+id).outerHeight()) + "px",
		        'left': (eICP.left) + "px",
		        'position': 'absolute'
		    }).fadeIn("fast");
		    ...
		    var def = jQuery("#" + elem.id).val();
		    jQuery('#colorPreview span').text(def);
		    jQuery('#colorPreview').css('color', def);
		        ...
		        for (j = 0; j < tblChilds.length; j++) {
		            ...
		            for (k = 0; k < tblCells.length; k++) {
		                jQuery(tblChilds[j].childNodes[k]).unbind().mouseover(function (a) {
		                    ...
		                    jQuery('#colorPreview').css('color', aaa);
		                    jQuery('#colorPreview span').text(aaa);
		                }).click(function () {
		                    var aaa = jQuery(this).attr('hx');
		                    jQuery("#" + id).val(aaa).css("color", '#'+aaa);
		                    jQuery("#iColorPickerBg").hide();
		                    jQuery("#iColorPicker").fadeOut();
		                    jQuery(this);
		                });
	...
	jQuery.fn.icolorpicker = function(conf) {
	  return this.each(function(){
		  var el = new iColorPicker(this, conf);
	      jQuery(this).data("iColorPicker", el);
	  });
	};
})();